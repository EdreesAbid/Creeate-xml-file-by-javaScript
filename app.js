...
async function renderWeekWeather() {
	...
	if(week==0){
		...
		try {
			...
			let html = '<div class="d-grid justify-content-md-end"><button type="button" onclick="generateXML()" class="btn btn-outline-dark">Generate XML</button></div>'+
...
async function generateXML(){
	//ge the selected week start and end date
	let week=document.getElementById("dropdownId").value;
	//return weather information form api
	let weatherInfos = await getWeatherInfoFromAPI(week);
	
	var xml = new xml4js({encoding: "ISO-8859-1"});
		xml.write("temperature", function() {
			let days=weatherInfos.daily.time;
			let maxTemps=weatherInfos.daily.temperature_2m_max;
			let minTemps=weatherInfos.daily.temperature_2m_min;
			...
			days.forEach((date, index) => {
				let dayName=weekday[new Date(date).getDay()];
				xml.write("day", dayName);
				xml.write("Date", function() {
					xml.write("dateValue",  date);
					xml.write("dateFormat", "YYYY-MM-DD");
				});
				xml.write("min", minTemps[index]);
				xml.write("max", maxTemps[index]);
			});
